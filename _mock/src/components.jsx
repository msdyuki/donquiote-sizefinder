// components.jsx — shared bits for all Donki screens
...
// Lightweight QR placeholder (pseudo-random grid, not a real QR)
function FakeQR({ size = 180, seed = 7 }) {
  const N = 21;
  const cells = [];
  let s = seed;
  for (let y = 0; y < N; y++) {
...
// Flag glyphs (emoji fallback + label)
const FLAGS = [
...
// Don-Ken (mascot) placeholder square — swap to <img> later
function DonKen({ size = 160, hat = true, style }) {
  return (
    <div className="donken-ph" style={{ width: size, height: size, ...style }} aria-label="Don-pen placeholder" />
  );
}

// Signage chrome wrapper (bezel-less full bleed) — adds marquee footer + lang chip + step indicator
function SignageChrome({ lang, setLang, step, totalSteps, showLang = false, children, footer }) {
  const t = STRINGS[lang] || STRINGS.jp;
  return (
    <>
      {/* top bar — brand ribbon */}
      <div style={{
      ...
      {/* footer band — STATIC, single line */}
      {footer !== false && (
...
// Full-stage wrapper (1080×1920)
function Stage({ children, bg = 'var(--donki-white)', style, ...rest }) {
  return (
...
Object.assign(window, {
  STRINGS, FLAGS, FakeQR, DonKen, SignageChrome, Stage,
});
